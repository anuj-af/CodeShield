...
async function AIRewrite(prompt) {
	const stream = await ai.models.generateContent({
		...
		contents: prompt,
		config: {
			systemInstruction: "You are code-rewriter, converting vulnerable code to a secure code. You just give a piece of code nothing else.",
		},
	});
...
async function NaturalQuery(prompt) {
	const stream = await ai.models.generateContent({
		...
		contents: prompt,
		config: {
			systemInstruction: "You are a natural language security query solver, explaining every query concisely",
		},
	});
	return stream.candidates[0].content.parts[0].text;
}
...
app.post("/query", async (req, res) => {
	const { message } = req.body;
	const data = await NaturalQuery(message);
	res.send(data);
});
